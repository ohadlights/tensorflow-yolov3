...
class yolov3(object):
    ...
    def compute_loss(self, pred_feature_map, y_true, ignore_thresh=0.5, max_box_per_image=8):
        ...
        # regularization loss
        reg_loss = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES, scope=tf.get_default_graph().get_name_scope())
        reg_loss = [tf.cast(v, tf.float32) for v in reg_loss]
        reg_loss = tf.reduce_sum(reg_loss)

        total_loss = loss_xy + loss_wh + loss_conf + loss_class + reg_loss
