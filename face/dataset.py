...
class Parser(object):
    ...
    def parser_example(self, file, gt_boxes):
...
class Dataset(object):
    def __init__(self, parser, list_path, batch_size, shuffle=None, repeat=True):
        self.parser = parser
        self.data = [l.strip().split() for l in open(list_path)]
    ...
    def _buildup(self):
        files = [l[0] for l in self.data]
        labels = [np.array([float(a) for a in l[1:]]) for l in self.data]

        # self.dataset = tf.data.Dataset.from_tensors((tf.constant(files), tf.constant(labels)))
        self.dataset = tf.data.Dataset.from_generator(lambda: zip(files, labels), output_types=(tf.string, tf.float32))

        self.dataset = self.dataset.map(map_func=self.parser.parser_example, num_parallel_calls=10)
        self.dataset = self.dataset.repeat() if self.repeat else self.dataset

        if self.shuffle is not None:
            self.dataset = self.dataset.shuffle(self.shuffle)
